...
const MOCK_TRIAGE_FINDINGS: Finding[] = [
  {
    id: "3e8da1c6-2193-41d4-9230-0514c6f2ea5e",
    modelName: "customers",
    severity: "HIGH",
    evidenceTier: "ORG-VALIDATED",
    summary: "Departing engineer J. Alvarez altered risk threshold logic mirroring prior incident (c3d4e5f6)",
    details: "Immediately revert threshold logic in the customers transformation, validate with code review, and reprocess downstream partitions.",
    precedent: "Backed by company-wide pattern history",
    precedentCount: 2,
    avgLagDays: 49,
    owner: "jonny1 (Data Owner)",
    hash: "3e8da...ea5e",
    actor: "J. Alvarez",
    changeDate: "2 days ago",
    lineagePath: "urn:li:dataset:(urn:li:dataPlatform:dbt,b2fd91.order_entry_db.order_entry.customers,PROD)",
  },
  {
    id: "98bd1ace-2a12-4148-a95c-9f56314df533",
    modelName: "addresses",
    severity: "HIGH",
    evidenceTier: "ORG-VALIDATED",
    summary: "Unvalidated zip code transformation bypasses schema validation rules",
    details: "Enforce strict schema validation rules across all address parsing nodes in dbt pipeline.",
    precedent: "Backed by company-wide pattern history",
    precedentCount: 2,
    avgLagDays: 66,
    owner: "jonny1 (Data Owner)",
    ...
    lineagePath: "urn:li:dataset:(urn:li:dataPlatform:dbt,b2fd91.order_entry_db.order_entry.addresses,PROD)",
  },
  {
    id: "a60b33ce-6491-4ee8-9d07-9c93f89c0766",
    modelName: "order_items",
    severity: "MEDIUM",
    evidenceTier: "ORG-VALIDATED",
    summary: "Quantity calculation logic change without corresponding unit tests",
    details: "Add comprehensive unit tests for quantity aggregations before merge.",
    precedent: "Backed by model-specific pattern history",
    precedentCount: 2,
    avgLagDays: 61,
    owner: "EMP006 (Platform Lead)",
    hash: "a60b3...0766",
    actor: "J. Alvarez",
    changeDate: "4 days ago",
    lineagePath: "urn:li:dataset:(urn:li:dataPlatform:dbt,b2fd91.order_entry_db.order_entry.order_items,PROD)",
  },
];

export default function RiskTriageList() {
  const [findings, setFindings] = useState<Finding[]>(MOCK_TRIAGE_FINDINGS);
  const [selectedId, setSelectedId] = useState<string>(MOCK_TRIAGE_FINDINGS[0].id);
  ...
  const handleEmitDataHub = async () => {
    ...
    setIsEmitting(true);
    // Showcase simulation only — no real network requests or backend state mutations on landing page
            ...
            {findings.map((finding) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-5 rounded-2xl border transition-all cursor-pointer select-none flex flex-col justify-between space-y-3 ${isSelected
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase border ${isHigh
                  ...
                  <div className="flex items-center justify-between text-[11px] text-zinc-500 pt-1">
                    <span
                      className={`px-2 py-0.5 rounded text-[9px] font-semibold uppercase ${finding.evidenceTier === "ORG-VALIDATED"
            ...
            <div className="lg:col-span-7 p-7 rounded-2xl border border-white/15 bg-gradient-to-b from-zinc-900/90 via-zinc-900/60 to-zinc-950/95 backdrop-blur-2xl shadow-2xl space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-800/80 pb-5">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`px-3 py-1 rounded-lg text-xs font-bold uppercase border ${selectedFinding.severity === "HIGH"
                  ...
                  <span
                    className={`px-3 py-1 rounded-lg text-xs font-semibold uppercase ${selectedFinding.evidenceTier === "ORG-VALIDATED"
