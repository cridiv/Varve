...
| Narrative synthesis (reasoning excluded — see Design Principles) | LLM API, model-agnostic by design |
| Backend | FastAPI (Python) |
| Frontend | React + TailwindCSS |
| Audit trail | Append-only SHA-256 hash-chained table, own verification script |
...
Five to six clean, hand-checkable scenarios that can be narrated from memory beat a much larger set that would need to be double-checked under Q&A pressure. Full detail in `docs/validation.md`.
...
```bash
# 1. Start DataHub and load the sample lineage graph
